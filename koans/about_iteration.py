...
class AboutIteration(Koan):
    ...
    def test_reduce_will_blow_your_mind(self):
        ...
        # given an operator, a starting value and a list of numbers, reduce
        # will apply the operator to the starting value and the first item, then the starting value will be updated to 
        # contain the result of the operation and then continues until the operator is applied to all the elements of the sequence
        # and then returns the updated starting value. The aim is to reduce the values to a single value which is determined by the operator used.
